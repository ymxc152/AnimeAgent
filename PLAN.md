...
| 对话层 | 规则意图解析 + `StatusQueryService` + `/api/chat` 端点 | `agents/conversational/`, `services/status_query.py`, `web.py` |
...
- `Episode` 同时存在 `torrent_hash` 与 `torrent_info_hash`：
  - 当前主流程使用 `torrent_hash`；
  - `torrent_info_hash` 保留用于兼容旧数据；
  - **下一步**：统一字段名并写迁移脚本。

### 3.3 架构债务

- `CompletionChecker` 外部状态分支不可达（代码逻辑问题，不影响主流程）。
...
1. **多轮对话上下文**
   - 在 `UserRequest` 表中维护对话历史；
   - 支持"刚才那个，下载全部"等指代消解。
2. **自然语言订阅**
   - 对话层识别 `subscribe` 意图后调用 `DiscoveryService` 创建 Subscription。
3. **通知渠道**

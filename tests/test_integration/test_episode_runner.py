"""Integration tests for the Episode Graph runner/executor."""
...
async def test_runner_loads_state_and_invokes_graph(db_session):
    ...
    runner = EpisodeGraphRunner(session_factory=AsyncMock(), graph=mock_graph)
...
async def test_runner_returns_resume_after(db_session):
    """Runner should return the graph's resume_after in its result."""
    ...
    assert final.get("resume_after") == resume_at.isoformat()
...
async def test_runner_is_callable_as_scheduler_executor(db_session):
    ...
    await store.episodes.create(
        Episode(subscription_id=sub.id, episode_number=1, status="pending")
    )
